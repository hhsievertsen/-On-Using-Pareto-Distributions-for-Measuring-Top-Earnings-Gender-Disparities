...
mle_pareto, income(v_log_income5) pareto_cutoff(0.1) savecoef("$of\coefficients_full_sample5.dta")

/* Create plots of estimated coefficients, gap and ceiling. */
preserve
	use "$of\coefficients_full_sample5.dta",replace
		*plot ceiling			
		tw (line gamma year, lcolor(black) lwidth(thick) ) ///
			...
			graph export "$of\glass_ceiling_time_series_5year.png", replace width(2000)
		*plot gap
	tw (line gap_ln year, lcolor(black) lwidth(thick) ) ///
		...
		graph export "$of\gender_gap_time_series_ln_5year.png", replace width(2000)
restore
